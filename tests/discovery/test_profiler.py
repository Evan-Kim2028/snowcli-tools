...
def _query_output(rows: list[dict] | None) -> QueryOutput:
    """Helper to wrap rows in QueryOutput for mocked run_query."""
    return QueryOutput(
        raw_stdout="",
        raw_stderr="",
        returncode=0,
...
class TestTableProfilerBasic:
    ...
    def test_profile_table_basic(self, profiler, mock_snow_cli):
        ...
        mock_snow_cli.run_query.side_effect = [
            _query_output([{"CNT": 1000}]),  # Row count
            _query_output([]),  # LAST_DDL (optional, will return None)
            _query_output(  # Column info
                [
                    {
                        "COLUMN_NAME": "ID",
            ...
            _query_output(  # Sample data
                [{"ID": 1, "NAME": "Alice"}, {"ID": 2, "NAME": "Bob"}]
            ),
            _query_output(  # ID stats
                [
                    {
                        "TOTAL_ROWS": 1000,
                        "NON_NULL_COUNT": 1000,
...
class TestPatternDetection:
    ...
    def test_no_pattern_for_non_string_columns(self, profiler, mock_snow_cli):
        ...
        mock_snow_cli.run_query.side_effect = [
            ...
            _query_output([{"AMOUNT": 123.45}, {"AMOUNT": 234.56}]),
            _query_output(
                [
                    {
                        "TOTAL_ROWS": 100,
...
class TestPerformanceAndScalability:
    ...
    def test_large_table_uses_approx_count(self, mock_snow_cli):
        ...
        mock_snow_cli.run_query.side_effect = [
            _query_output([{"CNT": 200000}]),  # Large table
            _query_output([]),
            _query_output(
                [
                    {
        ...
        calls = [str(call) for call in mock_snow_cli.run_query.call_args_list]
...
class TestEdgeCases:
    ...
    def test_table_with_all_null_column(self, profiler, mock_snow_cli):
        ...
        mock_snow_cli.run_query.side_effect = [
            ...
            _query_output([{"NULL_COL": None}, {"NULL_COL": None}]),
            _query_output(
                [
                    {
                        "TOTAL_ROWS": 100,
...
class TestErrorHandling:
    ...
    def test_table_not_found_raises_error(self, profiler, mock_snow_cli):
        ...
        mock_snow_cli.run_query.side_effect = [
            _query_output([{"CNT": 0}]),
            _query_output([]),
            _query_output([]),  # No columns found
        ]
    ...
    def test_sample_rows_respects_limit(self, profiler, mock_snow_cli):
        ...
        mock_snow_cli.run_query.side_effect = [
            ...
            _query_output([{"ID": i} for i in range(50)]),  # Sample data (limited)
            _query_output(
                [
                    {
                        "TOTAL_ROWS": 1000,
